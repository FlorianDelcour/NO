...
pi_G = 1245
pi_D = 23
delta_pi_G = -6.514360194162096
delta_pi_G = 0.4615883431355693
delta_pi_G = 0
...
# objective
@objective(model, Min, pi_PV*C_PV + pi_B*E_B_max + (pi_G + delta_pi_G)*C_G + pi_D*sum(P_D))

# constraints
@constraint(model, eq[t in time], P_PV[t] + n_G*P_D[t] + P_B_discharge[t] == P_B_charge[t] + P_C[t])
...
@constraint(model, der[t in time], E_B[t] <= E_B_max)
