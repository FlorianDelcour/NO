...
function lom(n_years, plt)
	...
	@constraint(model, [t in 2:n_hours], E_B[t] == n_B*E_B[t-1] + n_B_plus*P_B_charge[t-1] - P_B_discharge[t-1]/n_B_moins)
	...
	@constraint(model,[t = 2:n_hours], (P_D[t] - P_D[t-1])*n_G <= 0.1*C_G)
	@constraint(model,[t = 2:n_hours], (P_D[t] - P_D[t-1])*n_G >= - 0.1*C_G)
	...
	if plt == true
        ...
        savefig(string("Q7_Prod_consum_", n_years, "_year.pdf"))
        ...
        savefig(string("Q7_State_battery_", n_years, "_year.pdf"))
	end
end
